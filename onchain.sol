...
contract 0trust is SafeMath
{
    ...
    function getBalance(uint32[] prCode, address toFind) view public returns (uint128[] available, uint128[] reserved)
    {
        uint32 accountId = account_id[msg.sender];
        require(toFind == msg.sender || id_announceLV[accountId] > 0 || msg.sender == owner);
    ...
    {
        uint32 n = 0;
        for (uint32 i = 0; i < lastAcccountId; i++)
        {
    ...
    function getOrder(uint32 id) view public returns (uint32 prTrade, uint32 prBase, bool sell, uint80 price, uint104 qty)//, uint64 timestamp)
    {
        Order memory order = id_Order[id];
    ...
    function GetMyOrders() view public returns (uint32[] orderId, uint32[] prTrade, uint32[] prBase, bool[] sells, uint80[] prices, uint104[] qtys)//, uint64[] timestamp)
    {
        uint32 accountId = account_id[msg.sender];
